// Dialog_HowMany.cpp: implementation of the CDialog_HowMany class.
//
//////////////////////////////////////////////////////////////////////
...
CDialog_HowMany::CDialog_HowMany()
{
	m_bEnableFocus = TRUE;
}
...
BOOL CDialog_HowMany::OnButtonAction(int nSpec, int nIndex) 
{
...
BOOL CDialog_HowMany::Draw(CDDSurface *pDest)
{
...
BOOL CDialog_HowMany::Create(CDirectDraw *pDD, CString szLibName, int nType, BOOL bModal, BOOL bMoveable, BOOL bTrans)
{
...
void CDialog_HowMany::ShowWindow(int Id, CUIRes *pParent)
{
	if(!IsLoading())
	{
		CPoint t_pos;
		...
		SetWnd(g_pChildView);
		if(!Create(CChildView::DirectXMng.m_pDD, "Trash-I.lib", 0, TRUE, FALSE, FALSE))
		{
			return;
		}
...
BOOL CDialog_HowMany::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	if(nChar == VK_RETURN)
	{
		CString t_str;
...
void CDialog_HowMany::HideWindow()
{
	CUIRes::HideWindow();
...
void CDialog_HowMany::HideAllWindow()
{
	CUIRes::HideWindow();
