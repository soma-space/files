// Dialog_HowMuch.cpp: implementation of the CDialog_HowMuch class.
//
//////////////////////////////////////////////////////////////////////
...
CDialog_HowMuch::CDialog_HowMuch()
{
	m_bEnableFocus = TRUE;
}
...
BOOL CDialog_HowMuch::OnButtonAction(int nSpec, int nIndex) 
{
...
BOOL CDialog_HowMuch::Draw(CDDSurface *pDest)
{
...
BOOL CDialog_HowMuch::Create(CDirectDraw *pDD, CString szLibName, int nType, BOOL bModal, BOOL bMoveable, BOOL bTrans)
{
...
void CDialog_HowMuch::ShowWindow(int Id, CUIRes *pParent)
{
	if(!IsLoading())
	{
		CPoint t_pos;
		...
		SetWnd(g_pChildView);
		if(!Create(CChildView::DirectXMng.m_pDD, "Trash-M.lib", 0, TRUE, FALSE, FALSE))
		{
			return;
		}
...
BOOL CDialog_HowMuch::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	if(nChar == VK_RETURN)
	{
		CString t_str;
...
void CDialog_HowMuch::HideWindow()
{
	CUIRes::HideWindow();
...
void CDialog_HowMuch::HideAllWindow()
{
	CUIRes::HideWindow();
