...
void CAniObj::AniLoad(const char* szFileName)
{
	...
	_ANI_HEADER HAni;
	ReadFile(hFile, &HAni, sizeof(_ANI_HEADER), &dwCount, NULL); // 에니메이션 헤더 읽기
	if(HAni.nDirQt <= 0 || HAni.szID[0] != 'A' || HAni.szID[1] != 'N' || HAni.szID[2] != 'I')
	{
	...
	for(int i = 0; i < m_HAni.nQt; i++)
	{
		...
		{
			m_pAnis[i]->pnFrms = new short int[m_pAnis[i]->nMaxFrm*HAni.nDirQt];
			ReadFile(hFile, m_pAnis[i]->pnFrms, m_pAnis[i]->nMaxFrm*HAni.nDirQt*2, &dwCount, NULL);
		}
	}
	CloseHandle(hFile);
	// 에니메이션...................................................................
...
void CAniObj::AniLoadEx(const char* szFileName, BOOL bPart )//this_obj_load
{
	if( bPart )
	{
		if( m_HAni.nQt > 0 ) return;
	}
...
void CAniObj::AniCtrlAllocEx(int nQt, BOOL bPart)
{
	if( bPart )
	{
		if( m_nCtrlQt == 256 ) return;
	}
	if(nQt <= 0 || nQt > MAX_ANI_CTRL) return;
...
int CAniObj::FrmGetAnimated(int nCtrl)
{
	...
	if(m_pAnis[nCtrl] == NULL) return -1;
	int nMaxFrm = m_pAnis[nCtrl]->nMaxFrm;
	if(nMaxFrm <= 0 || nMaxFrm > MAX_FRM) return -1;
	
	if(GetTickCount() > m_dwTickFrms[nCtrl] + (1000 / m_pAnis[nCtrl]->fSpd * m_fASpeed)) // 시간에 따라 제어...
	{
	...
	return m_pAnis[nCtrl]->pnFrms[nDir*nMaxFrm + nFrm];
