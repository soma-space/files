// DDEngine.cpp: implementation of the CDDEngineNew class.
//
//////////////////////////////////////////////////////////////////////
...
void CDDEngineNew::Init(HWND hWnd, BOOL bFullScreen, int nWidth, int nHeight)
{
...
//	rval = m_lpDD->QueryInterface(IID_IDirectDraw4, (void**)&m_lpDD4);
//	if(rval != DD_OK) { MessageBox(m_hWnd, "DirectX 4 Not Installed. or Incorrect.", "Initialization Error", MB_OK); exit(-1);}
//	RELEASE_SAFE(lpDD1);

//	rval = lpDD1->QueryInterface(IID_IDirectDraw7, (void**)&m_lpDD4);
//	if(rval != DD_OK) { MessageBox(m_hWnd, "DirectX 7 Not Installed. or Incorrect.", "Initialization Error", MB_OK); exit(-1);}
//	RELEASE_SAFE(lpDD1);
	rval = DirectDrawCreateEx(NULL, (void **)&m_lpDD4, IID_IDirectDraw7, NULL);
	if(rval != DD_OK) { MessageBox(m_hWnd, "DirectX 7 Not Installed. or Incorrect.", "Initialization Error", MB_OK); exit(-1);}

	DDSCAPS2		ddscaps; DDSURFACEDESC2	ddsd;
	...
	{
		rval = m_lpDD4->SetCooperativeLevel(m_hWnd, DDSCL_ALLOWREBOOT | DDSCL_EXCLUSIVE | DDSCL_FULLSCREEN);
		if (rval != DD_OK)
		{
		...
		rval = m_lpDD4->SetDisplayMode(m_rcScr.right, m_rcScr.bottom, 16, 0,0);
		if (rval != DD_OK)
		{
		...
		rval = m_lpDD4->CreateSurface(&ddsd, &m_lpDDSPri, NULL); // Create the primary surface
	...
	{
		rval = m_lpDD4->SetCooperativeLevel(m_hWnd, DDSCL_NORMAL);
		// Primary 버퍼 생성
		MAKE_ZERO(ddscaps);
		MAKE_ZERO(ddsd);
		ddsd.dwSize         = sizeof(ddsd);
...
//		ddsd.ddsCaps.dwCaps = DDSCAPS_OFFSCREENPLAIN | DDSCAPS_VIDEOMEMORY;
		rval = m_lpDD4->CreateSurface(&ddsd, &m_lpDDSBB, NULL); // Create the rendering surface
...
LPDIRECTDRAWSURFACE7 CDDEngineNew::SurfaceCreate(int nWidth, int nHeight, BOOL bUseVideoMemory, COLORREF ColorKey)
{
	if(m_lpDD4 == NULL) return NULL;
    ...
    if(bUseVideoMemory == TRUE) // 비디오 메모리에 생성할 꺼면...
	{
		...
		if (m_lpDD4->CreateSurface(&ddsd, &lpdds, NULL) != DD_OK) // 비디오 메모리 부족.. 시스템 메모리에 생성
		{
			ddsd.ddsCaps.dwCaps = DDSCAPS_OFFSCREENPLAIN | DDSCAPS_SYSTEMMEMORY;
			if (m_lpDD4->CreateSurface(&ddsd, &lpdds, NULL) != DD_OK) return NULL;
		}
	}
	else
	{
		ddsd.ddsCaps.dwCaps = DDSCAPS_OFFSCREENPLAIN | DDSCAPS_SYSTEMMEMORY;
		m_lpDD4->CreateSurface(&ddsd, &lpdds, NULL);
	}
...
void CDDEngineNew::SurfaceColorKeySet(LPDIRECTDRAWSURFACE7 lpdds, COLORREF ColorKey)
{
...
// nAlpha - 0 ~ 32
void CDDEngineNew::BltNormal(LPDIRECTDRAWSURFACE7 lpDDSSrc,
						  int x,
						  int y,
						  const RECT *prcSrc,
						  BOOL bUseColorKey,
...
void CDDEngineNew::BackBufferLock()
{
...
void CDDEngineNew::BackBufferUnlock()
{
...
LPDIRECTDRAWSURFACE7 CDDEngineNew::SurfaceCreateWithBMP(const char *szFileName, BOOL bUseVideoMemory, COLORREF crCK ,BOOL bSetting)
{
	HANDLE hFile;
	DWORD dwCount;

	char szTmp[256];
	wsprintf(szTmp, "bmp\\%s",szFileName);
	hFile = CreateFile( szTmp, GENERIC_READ, 0, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
	...
	if(szID[0] == 'B' && szID[1] == 'M' && (szID[2] == 'P' || szID[2] == 'C') && szID[3] == 16)
		return this->SurfaceCreateWith16BMP(szTmp, bUseVideoMemory, crCK, bSetting);
	else if(szID[0] == 'B' && szID[1] == 'M')
		return this->SurfaceCreateWithGenericBMP(szTmp, bUseVideoMemory, crCK);
	else
	{
...
LPDIRECTDRAWSURFACE7 CDDEngineNew::SurfaceCreateWithGenericBMP(const char *szFileName, BOOL bUseVideoMemory, COLORREF crCK)
{
	if(m_lpDD4 == NULL) return NULL;
...
LPDIRECTDRAWSURFACE7 CDDEngineNew::SurfaceCreateWith16BMP(const char *szFileName, BOOL bUseVideoMemory, COLORREF crCK,BOOL bSetting)
{
	CDib16 dib;
	...
	// 서피스를 만든다.
	LPDIRECTDRAWSURFACE7 lpDDS = this->SurfaceCreate(width, height, bUseVideoMemory, crCK);
...
//		if(m_nPF == PF_565) memcpy(pDest + y * nDestPitch, pSrc + y * width, width*2); // 가로 픽셀 수만큼 복사..
		...
		{
			if( bSetting )
			 Conv565_1SetPixel(pDest + y * nDestPitch, pSrc + y * width, width); // 가로 픽셀 수만큼 복사..
			else
			 Conv565_0SetPixel(pDest + y * nDestPitch, pSrc + y * width, width); // 가로 픽셀 수만큼 복사..
		}
		else
		{
			if( bSetting )
			Conv565_1PixelTo555Pixel(pDest + y * nDestPitch, pSrc + y * width, width); // 가로 픽셀 수만큼 565 를 555 로 변환한다..
			else
			Conv565_0PixelTo555Pixel(pDest + y * nDestPitch, pSrc + y * width, width); // 가로 픽셀 수만큼 565 를 555 로 변환한다..
		}
	}
	dib.Unlock();
	SurfaceUnlock(lpDDS);
...
const char* CDDEngineNew::PathGetCur(const char* szPath, int nPath) const
{
...
// 현재 경로를 지정
void CDDEngineNew::PathSetCur(const char *szSubPath)
{
...
// 작업 경로 지정
void CDDEngineNew::PathSetWork(const char *szPath)
{
	lstrcpy(m_szPath, szPath);
}

void CDDEngineNew::Flip(const RECT* pRCClip)
{
...
void CDDEngineNew::SurfaceFillRect(LPDIRECTDRAWSURFACE7 lpDDS, const RECT* pRC, COLORREF crFill, int nAlpha)
{
...
// 서피스 사이즈를 렉트값으로 돌려줌..
void CDDEngineNew::SurfaceGetSize(LPDIRECTDRAWSURFACE7 lpDDS, RECT* pRC)
{
	if(lpDDS == NULL)
	{
		SetRect(pRC, 0, 0, 0, 0);
	...
	m_ddsdTmp1.dwSize = sizeof(DDSURFACEDESC2);
...
void CDDEngineNew::DrawLine(LPDIRECTDRAWSURFACE7 lpDDS, int sx, int sy, int ex, int ey, COLORREF crColor)
{
	if(lpDDS == NULL) lpDDS = m_lpDDSBB;
...
void CDDEngineNew::DrawBox(LPDIRECTDRAWSURFACE7 lpDDS, const RECT *pRC, COLORREF crColor)
{
	if(lpDDS == NULL) lpDDS = m_lpDDSBB;
	if(pRC == NULL) return;
...
void CDDEngineNew::DrawRhombus(LPDIRECTDRAWSURFACE7 lpDDS, int x, int y, COLORREF crColor)
{
	if(lpDDS == NULL) lpDDS = m_lpDDSBB;
...
void CDDEngineNew::DrawPolygon(LPDIRECTDRAWSURFACE7 lpDDS, POINT *point, int count, COLORREF crColor)
{
	if(lpDDS == NULL) lpDDS = m_lpDDSBB;
...
void CDDEngineNew::TextOut(LPDIRECTDRAWSURFACE7 lpDDS, int x, int y, const char *szText, COLORREF crText, UINT align)
{
	if(lpDDS == NULL) lpDDS = m_lpDDSBB;
...
COLORREF CDDEngineNew::SurfaceColorKeyGet(LPDIRECTDRAWSURFACE7 lpDDS)
{
...
void CDDEngineNew::Blt(LPDIRECTDRAWSURFACE7 lpDDSDest, LPDIRECTDRAWSURFACE7 lpDDSSrc, RECT *pRCDest, RECT *pRCSrc, BOOL bUseColorKey)
{
	if(lpDDSSrc == NULL) return;
