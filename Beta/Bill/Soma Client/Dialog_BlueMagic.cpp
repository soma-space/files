// Dialog_BlueMagic.cpp: implementation of the CDialog_BlueMagic class.
//
//////////////////////////////////////////////////////////////////////
...
CDialog_BlueMagic::CDialog_BlueMagic(CWnd *wnd)
:CUIRes(wnd)
{
...
CDialog_BlueMagic::~CDialog_BlueMagic()
{
...
BOOL CDialog_BlueMagic::Create(CDirectDraw *pDD, CString szLibName, int nType, BOOL bModal, BOOL bMoveable, BOOL bTrans)
{
...
BOOL CDialog_BlueMagic::Draw(CDDSurface *pDest)
{
...
BOOL CDialog_BlueMagic::DragDraw(CDDSurface *pDest)
{
	CRect rtSrc;
	CUser *me = CChildView::UserManager.GetMe();
	...
	if(me && m_vDragItem >= 0)
	{
		...
		rtSrc = Item_Total.m_Magic_Item->m_SprData[me->m_arrBlueMagic[m_vDragItem]->m_vNumber-1].rcV;
...
BOOL CDialog_BlueMagic::OnButtonAction(int nSpec, int nIndex)
{
	if(nSpec == BTN_CTRL)
	{
		switch(nIndex)
...
void CDialog_BlueMagic::SetScrollData()
{
	int max_line;
	int count;
	CUser *me;
	...
	count = me->m_arrBlueMagic.GetSize();
	m_vMagicCount = count;
...
void CDialog_BlueMagic::InitData()
{
	m_pScrollBar->InitScroll();
	SetScrollData();
}

BOOL CDialog_BlueMagic::OnLButtonDown(BBDX_MouseInformation *pMouseInfo)
{
	...
	if(retVal)
	{
		if(m_vSpecCtrl == AREA_CTRL)
		{
			if(m_vIndexCtrl >= 0 && m_vIndexCtrl <= 4)
			{
				...
				if(item_index < me->m_arrBlueMagic.GetSize())
				{
...
BOOL CDialog_BlueMagic::OnMouseMove(BBDX_MouseInformation *pMouseInfo)
{
...
BOOL CDialog_BlueMagic::OnLButtonUp(BBDX_MouseInformation *pMouseInfo)
{
	if(m_vDragItem >= 0)
	{
		...
		for(i = 26; i <= 29; i++)
		{
			if(Dialog_Main.m_arrArea[i]->m_vRect.PtInRect(m_pos))
			{
				...
				me = CChildView::UserManager.GetMe();
				Dialog_Main.SetMagicData(i-26, me->m_arrBlueMagic[m_vDragItem]);
			}
		}
		m_vDragItem = -1;
	}
...
void CDialog_BlueMagic::ShowWindow()
{
	if(!IsLoading())
	{
		if (!Create(CChildView::DirectXMng.m_pDD, "Magic-B.lib", 0, FALSE, FALSE))
		{
			return;
		}
...
void CDialog_BlueMagic::HideWindow()
{
	g_pShowDialog = NULL;
...
void CDialog_BlueMagic::HideAllWindow()
{
	m_pDialogCtrl->HideWindow();
}

BOOL CDialog_BlueMagic::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	if(nChar == VK_ESCAPE)
	{
		m_pDialogCtrl->HideWindow();
