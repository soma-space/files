// Dialog_Message3.cpp: implementation of the CDialog_Message3 class.
//
//////////////////////////////////////////////////////////////////////
...
CDialog_Message3::CDialog_Message3()
{
//	INIT_PTR(m_pTWFont);
//	INIT_PTR(m_pOuter);
	m_pFont = NULL;
...
BOOL CDialog_Message3::Create(CDirectDraw *pDD, CString szLibName, int nType, BOOL bModal, BOOL bMoveable, BOOL bTrans)
{
...
/*	m_pTWFont = new CTWFont();
	if (!m_pTWFont->CreateFontWithoutPath("Soma14", 14, 14))
	{
		...
		TRACE("####### Dialog Message3 Init ½ÇÆÐ.. ############ \n");
		return FALSE;
	}
...
BOOL CDialog_Message3::Draw(CDDSurface *pDest)
{
...
BOOL CDialog_Message3::OnButtonAction(int nSpec, int nIndex)
{
	if(nSpec == BTN_CTRL)
	{
		if(nIndex == 0) // Cancel
...
void CDialog_Message3::SetMessage(CString &message)
{
/*	char *t_buf;
	char seps[] = "\n";
	char *token;
...
void CDialog_Message3::ShowWindow(int Id, CUIRes *pParent)
{
	if(!IsLoading())
	{
		CPoint t_pos;

		if(!Create(CChildView::DirectXMng.m_pDD, "message3.lib", 0, TRUE, FALSE, FALSE))
		{
			return;
		}
...
void CDialog_Message3::HideWindow()
{
	if(m_pParent)
	{
...
void CDialog_Message3::HideAllWindow()
{
	if(m_pParent)
	{
		m_pParent->m_pChild = NULL;
...
BOOL CDialog_Message3::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
