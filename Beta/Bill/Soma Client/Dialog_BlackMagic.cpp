// Dialog_BlackMagic.cpp: implementation of the CDialog_BlackMagic class.
//
//////////////////////////////////////////////////////////////////////
...
CDialog_BlackMagic::CDialog_BlackMagic(CWnd *wnd)
:CUIRes(wnd)
{
...
CDialog_BlackMagic::~CDialog_BlackMagic()
{
...
BOOL CDialog_BlackMagic::Create(CDirectDraw *pDD, CString szLibName, int nType, BOOL bModal, BOOL bMoveable, BOOL bTrans)
{
...
BOOL CDialog_BlackMagic::Draw(CDDSurface *pDest)
{
...
BOOL CDialog_BlackMagic::DragDraw(CDDSurface *pDest)
{
	CRect rtSrc;
	CUser *me = CChildView::UserManager.GetMe();
	...
	if(me && m_vDragItem >= 0)
	{
		...
		rtSrc = Item_Total.m_Magic_Item->m_SprData[me->m_arrBlackMagic[m_vDragItem]->m_vNumber-1].rcV;
...
BOOL CDialog_BlackMagic::OnButtonAction(int nSpec, int nIndex)
{
	if(nSpec == BTN_CTRL)
	{
		switch(nIndex)
...
void CDialog_BlackMagic::SetScrollData()
{
	int max_line;
	int count;
	CUser *me;
	...
	count = me->m_arrBlackMagic.GetSize();
...
void CDialog_BlackMagic::InitData()
{
	m_pScrollBar->InitScroll();
	SetScrollData();
}

BOOL CDialog_BlackMagic::OnLButtonDown(BBDX_MouseInformation *pMouseInfo)
{
	...
	if(retVal)
	{
		if(m_vSpecCtrl == AREA_CTRL)
		{
			if(m_vIndexCtrl >= 0 && m_vIndexCtrl <= 4)
			{
				...
				if(item_index < me->m_arrBlackMagic.GetSize())
				{
...
BOOL CDialog_BlackMagic::OnMouseMove(BBDX_MouseInformation *pMouseInfo)
{
...
BOOL CDialog_BlackMagic::OnLButtonUp(BBDX_MouseInformation *pMouseInfo)
{
	if(m_vDragItem >= 0)
	{
		...
		for(i = 26; i <= 29; i++)
		{
			if(Dialog_Main.m_arrArea[i]->m_vRect.PtInRect(m_pos))
			{
				...
				me = CChildView::UserManager.GetMe();
				Dialog_Main.SetMagicData(i-26, me->m_arrBlackMagic[m_vDragItem]);
			}
		}
		m_vDragItem = -1;
	}
...
void CDialog_BlackMagic::ShowWindow()
{
	if(IsLoading() == FALSE)
	{
		if (!Create(CChildView::DirectXMng.m_pDD, "Magic-D.lib", 0, FALSE, FALSE))
		{
			return;
		}
	}
...
void CDialog_BlackMagic::HideWindow()
{
	g_pShowDialog = NULL;
...
void CDialog_BlackMagic::HideAllWindow()
{
	m_pDialogCtrl->HideWindow();
}

BOOL CDialog_BlackMagic::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	if(nChar == VK_ESCAPE)
	{
		m_pDialogCtrl->HideWindow();
