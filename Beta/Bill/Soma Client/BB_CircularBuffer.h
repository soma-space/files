...
{
public:
	BB_CircularBuffer(int size=8192);
	...
	const BYTE	*GetCoreData(int& buf_size) {buf_size = m_iBufSize; return m_pBuffer;};
	//1 Byte Operation;
...
{
	if (len<=0) {TRACE("BB_CircularBuffer::PutData len is <=0\n"); return;}
	while (IsOverFlowCondition(len)) BufferResize();
