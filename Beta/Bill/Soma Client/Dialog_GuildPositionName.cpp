// Dialog_GuildPositionName.cpp: implementation of the CDialog_GuildPositionName class.
//
//////////////////////////////////////////////////////////////////////
...
CDialog_GuildPositionName::CDialog_GuildPositionName()
{
	m_bEnableFocus = TRUE;
}
...
BOOL CDialog_GuildPositionName::OnButtonAction(int nSpec, int nIndex) 
{
...
BOOL CDialog_GuildPositionName::Draw(CDDSurface *pDest)
{
...
BOOL CDialog_GuildPositionName::Create(CDirectDraw *pDD, CString szLibName, int nType, BOOL bModal, BOOL bMoveable, BOOL bTrans)
{
...
void CDialog_GuildPositionName::ShowWindow(int Id, CUIRes *pParent, int str_len)
{
	if(!IsLoading())
	{
		CPoint t_pos;
		...
		SetWnd(g_pChildView);
		if(!Create(CChildView::DirectXMng.m_pDD, "Position.lib", 0, TRUE, FALSE, FALSE))
		{
			return;
		}
...
BOOL CDialog_GuildPositionName::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	if(nChar == VK_RETURN)
	{
...
void CDialog_GuildPositionName::HideWindow()
{
	CUIRes::HideWindow();
...
void CDialog_GuildPositionName::HideAllWindow()
{
	CUIRes::HideWindow();
...
void CDialog_GuildPositionName::SetName(CString name)
{
