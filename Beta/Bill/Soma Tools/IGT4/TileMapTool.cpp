...
void CTileMapTool::LoadFromToolData(const char* szFileName)
{
	...
	DWORD *st_buf;
	DWORD i, j, k, l;
/*	struct OLD_TILE_MAP_HEADER
	{
		char szID[4]; // 'T' 'M' 'D' NULL;
		char szRemark[64]; // 이름??
		int nWidth; // -1 이면 쓰지 않는다..
		int nHeight; // -1 이면 쓰지 않는다..
		char szBMPFNs[5][64]; // 비트맵 파일 이름... 게임 데이터에서만 쓰인다.
		COLORREF crCKs[5]; // 컬러키
		int nResrcQts[5]; // 타일 리소스 갯수..
	} old_tile_map_header;*/
...
/*	CopyMemory(m_HTMap.szID,, old_tile_map_header.szID, 4);
	CopyMemory(m_HTMap.szRemark, old_tile_map_header.szRemark, 64);
	m_HTMap.nWidth = old_tile_map_header.nWidth;
	m_HTMap.nHeight = old_tile_map_header.nHeight;
	CopyMemory(&m_HTMap.szBMPFNs[0], &old_tile_map_header.szBMPFNs[0], 64);
	CopyMemory(&m_HTMap.szBMPFNs[1], &old_tile_map_header.szBMPFNs[1], 64);
	CopyMemory(&m_HTMap.szBMPFNs[3], &old_tile_map_header.szBMPFNs[2], 64);
	CopyMemory(&m_HTMap.szBMPFNs[4], &old_tile_map_header.szBMPFNs[3], 64);
	CopyMemory(&m_HTMap.szBMPFNs[5], &old_tile_map_header.szBMPFNs[4], 64);
	m_HTMap.crCKs[0] = old_tile_map_header.crCKs[0];
	m_HTMap.crCKs[1] = old_tile_map_header.crCKs[1];
	m_HTMap.crCKs[3] = old_tile_map_header.crCKs[2];
	m_HTMap.crCKs[4] = old_tile_map_header.crCKs[3];
	m_HTMap.crCKs[5] = old_tile_map_header.crCKs[4];
	m_HTMap.nResrcQts[0] = old_tile_map_header.nResrcQts[0];
	m_HTMap.nResrcQts[1] = old_tile_map_header.nResrcQts[1];
	m_HTMap.nResrcQts[3] = old_tile_map_header.nResrcQts[2];
	m_HTMap.nResrcQts[4] = old_tile_map_header.nResrcQts[3];
	m_HTMap.nResrcQts[5] = old_tile_map_header.nResrcQts[4];*/
...
/*				if(_tmd.tsn >= MAX_TILE_SET)
				{
					if(_tmd.tsn != 0 || _tmd.tssn != 0 || _tmd.num != 4095)
					{
						TRACE3("%d - (%d, %d) :", i, k%850, k/850);
						TRACE3("%d,%d,%d\n", _tmd.tsn, _tmd.tssn, _tmd.num);
					}
					m_pDatasTool[i][k].snTS = -1;
					m_pDatasTool[i][k].snTSS = -1;
					m_pDatasTool[i][k].snNum = -1;
				...
				LPDIRECTDRAWSURFACE lpDDS = g_pTSet->m_lpDDSs[_tmd.tsn];
				if(lpDDS == NULL)
				{
					if(_tmd.tsn != 0 || _tmd.tssn != 0 || _tmd.num != 4095)
					{
				...
				POINT *t_pos = g_pTSet->DataGetPosCur(_tmd.tsn, _tmd.tssn, _tmd.num);
				int *t_flag = g_pTSet->DataGetFlagCur(_tmd.tsn, _tmd.tssn, _tmd.num);
				if(t_pos == NULL || t_flag == NULL ||
					t_pos->x < 0 ||
					t_pos->x >= MAX_TILE_SET_WIDTH ||
					t_pos->y < 0 ||
					t_pos->y >= MAX_TILE_SET_HEIGHT)
				{
					if(_tmd.tsn != 0 || _tmd.tssn != 0 || _tmd.num != 4095)
					{
						TRACE3("%d - (%d, %d) :", i, k%850, k/850);
	...
	offset = 0;
	for(i = 3; i < MAX_TILE_MAP_LAYER; i++) // 툴에서 쓴 데이터 저장...
	{
		table_count = st_buf[offset++];
		k = 0;
		for(j = 0; j < table_count; j++)
