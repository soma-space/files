...
void CWndAniObj::OnPaint() 
{
	...
	if(nWork == 0) // 수동 스프라이트 설정 작업...
	{
		if(m_Dlg.m_pAniObj->m_lpDDS != NULL) // 작업용 서피스가 있으면..
		{
			...
			if(m_Dlg.IsDlgButtonChecked(IDC_C_TILE_SET))//this_plus
			{
				CRect rcClinet;
				GetClientRect(&rcClinet);
				g_pTEng->DrawGrid(rcClinet,RGB(0,0,0));
			}
...
void CWndAniObj::OnLButtonDown(UINT nFlags, CPoint point) 
{
	...
	if(m_Dlg.IsDlgButtonChecked(IDC_C_TILE_SET))//this_plus
	{
		pt = g_pTEng->CalcTileSetPoint(pt); 
	}
	...
	if(m_Dlg.IsDlgButtonChecked(IDC_C_TILE_SET))//this_plus
	{
		point = g_pTEng->CalcTileSetPoint(point); 
	}
...
//this_plus_start
	CPoint ptCenter;
	if(m_Dlg.IsDlgButtonChecked(IDC_C_TILE_SET))
	{
		ptCenter.x = rc.left;
		ptCenter.y = rc.top;
	}
	else
		ptCenter = g_pTEng->FindCenterAuto(m_Dlg.m_pAniObj->m_lpDDS, rc, crCenter);
//this_plus_end
