...
BOOL CDib16::ImportGenericBMP(const char* szFileName)
{
	...
	DWORD pixel_width = bmHInfo.biWidth*dwPixelSize;
	if(pixel_width % 4) pixel_width += (4 - pixel_width%4);
	for(int y = 0; y < bmHInfo.biHeight; y++)
	{
		unsigned char* pbiTmpSrc = pbiSrc + (bmHInfo.biHeight - y - 1) * pixel_width;//(bmHInfo.biWidth*dwPixelSize + (bmHInfo.biWidth*dwPixelSize)%2); // 비트맵은 거꾸로 있다..
...
BOOL CDib16::ExportTo24BMP(const char *szFileName)
{
	...
	int nPitch = m_Header.nWidth*3;
	if(nPitch%4) nPitch = nPitch + (4 - nPitch%4);
...
BOOL CDib16::Load(const char* szFileName)
{
	...
	if(Header.szID[0] != 'B' || Header.szID[1] != 'M' || Header.szID[2] != 'P', Header.szID[3] != 16)
	{
		CloseHandle(hFile);
		return FALSE;
	}
