...
CPoint CDDEngineTool4::CalcTileSetPoint(CPoint pt)//this_plus
{
	int x_share  = pt.x / 32;
	int y_share  = pt.y / 32;
	int x_offset = pt.x % 32;
	int y_offset = pt.y % 32;
	if( x_offset > 16 ) x_share++;
	if( y_offset > 16 ) y_share++;
	pt.x = x_share*32;
	pt.y = y_share*32;
	return pt;
}

void CDDEngineTool4::DrawGrid( CRect rcClient, COLORREF rgb )//this_plus
{
	...
	for(int x = x1; x <= x2; x++)
	{
		this->DrawLine(NULL, x*TILE_SIZE, rcClient.top, x*TILE_SIZE, rcClient.bottom, rgb);
	}
	for(int y = y1; y <= y2; y++)
	{
		this->DrawLine(NULL, rcClient.left, y*TILE_SIZE, rcClient.right, y*TILE_SIZE, rgb);
	}
}
