// DlgLookEdit.cpp : implementation file
//

#include "stdafx.h"
#include "igt4.h"
#include "DlgLookEdit.h"
...
CDlgLookEdit::CDlgLookEdit(CWnd* pParent /*=NULL*/)
	: CDialog(CDlgLookEdit::IDD, pParent)
{
	m_ptCenter.x = 0; m_ptCenter.y = 0;
	MAKE_ZERO(m_nPartIndices);
	m_pLookClip = NULL;
	
	//{{AFX_DATA_INIT(CDlgLookEdit)
	//}}AFX_DATA_INIT
}

CDlgLookEdit::~CDlgLookEdit()
{
	DELETE_SAFE(m_pLookClip);
}

void CDlgLookEdit::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CDlgLookEdit)
...
BEGIN_MESSAGE_MAP(CDlgLookEdit, CDialog)
	//{{AFX_MSG_MAP(CDlgLookEdit)
...
/////////////////////////////////////////////////////////////////////////////
// CDlgLookEdit message handlers

void CDlgLookEdit::OnStaticPreview() 
{
...
void CDlgLookEdit::UpdatePreview()
{
	CRect rcPreview;
...
void CDlgLookEdit::OnBBrowseSpr() 
{
	CString FileName;
	DWORD dwFlags = OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT;
...
void CDlgLookEdit::OnBBrowseAni() 
{
	CString FileName;
	DWORD dwFlags = OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT;
...
void CDlgLookEdit::OnSelchangeListAni()
{
	this->UpdateLookData();
...
BOOL CDlgLookEdit::OnInitDialog() 
{
...
void CDlgLookEdit::UpdateAniList()
{
...
void CDlgLookEdit::OnCAnimation() 
{
...
void CDlgLookEdit::OnTimer(UINT nIDEvent) 
{
	if(nIDEvent == 0) // 재생..
	{
		this->UpdatePreview();
...
void CDlgLookEdit::OnBLookQtSet()
{
...
void CDlgLookEdit::OnSelchangeListPartAni() 
{
	int nPart = m_CBPart.GetCurSel();
...
void CDlgLookEdit::OnSelchangeCbPart() 
{
	this->UpdatePartAniList();
	this->UpdatePreview();
}

void CDlgLookEdit::OnSelchangeCbLook() 
{
	this->UpdatePartComboBox();
	this->UpdatePreview();
}

void CDlgLookEdit::OnBPartQtSet()
{
	this->UpdateLookData();
	this->UpdateLookInfo();
	this->UpdatePreview();
...
void CDlgLookEdit::UpdateLookData()
{
	int nLook = m_CBLook.GetCurSel();
	if(nLook < 0 || nLook >= MAX_LOOK) return;
	_LOOK_DATA* pData = m_ObjLook.DataGet(nLook);
...
void CDlgLookEdit::UpdateLookInfo()
{
	int nLook = m_CBLook.GetCurSel();
	_LOOK_DATA* pData = m_ObjLook.DataGet(nLook);
	if(pData== NULL) return;
...
void CDlgLookEdit::UpdatePartAniList()
{
	int nLook = m_CBLook.GetCurSel();
	_LOOK_DATA* pData = m_ObjLook.DataGet(nLook);
	if(pData== NULL) return;
...
void CDlgLookEdit::UpdatePartComboBox()
{
	this->UpdatePartAniList();
}

void CDlgLookEdit::UpdateLookComboBox()
{
