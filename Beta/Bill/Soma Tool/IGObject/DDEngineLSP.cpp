...
void CDDEngineLSP::BltLSP(LPDIRECTDRAWSURFACE lpDDSDest, int x, int y, const _SPR_LSP* pSprLSP, int nRenderType, int nAlpha, COLORREF crColor)
{
	...
	if(rc.right < 0 || rc.bottom < 0 || rc.left >= m_rcScr.right || rc.top >= m_rcScr.bottom)
	{
		if(bNeedUnlockDDS) SurfaceUnlock(lpDDSDest);
		return; // 화면을 완전히 벗어남..
	}
...
void CDDEngineLSP::BltLSPOff(LPDIRECTDRAWSURFACE lpDDSDest, int x, int y, SIZE pSize, const _SPR_LSP* pSprLSP, int nRenderType, int nAlpha, COLORREF crColor)
{
	if(pSprLSP == NULL) return;
	if(pSprLSP->pwV == NULL) return; // LSP 데이터가 없으면 돌아간다..
	if(pSprLSP->rcV.right - pSprLSP->rcV.left <= 0 || pSprLSP->rcV.bottom - pSprLSP->rcV.top <= 0) return; // 유효한 픽셀이 없으면 돌아간다..
	...
	if(rcClip.right > pSize.cx)
	{
		rcClip.right = pSize.cx;
		bUseXClip = TRUE;
	}
	if(rcClip.top < 0) rcClip.top = 0;
	if(rcClip.bottom > pSize.cy) rcClip.bottom = pSize.cy;
