...
protected:
	...
	DDSURFACEDESC m_ddsdTmp1, m_ddsdTmp2; // 임시로만 쓰이는 서피스 구조체.. -> 정말 임시로만 쓰여야 한다..
...
public:
	// 예비 버퍼 생성.
	LPDIRECTDRAWSURFACE m_lpDDSExtra; // 예비용 버퍼.. lsp 스프라이트를 작게 찍는 다거나 할때 쓴다.
	HDC m_hDC;
...
public: // 인라인 함수만 모아두자..
	...
	HRESULT SurfaceLock(LPDIRECTDRAWSURFACE lpDDS, DDSURFACEDESC* pDDSD, RECT* pRC = NULL) // 서피스 락.. DMA 이용은 반드시 시스템 메모리에 생성되었을 경우에만 한다..
	{
		MAKE_ZERO(*pDDSD);
	...
	void Conv565PixelTo555Pixel(WORD* pDest, const WORD* pSrc, int nCount) // 565 픽셀을 555 픽셀로 카운트 만큼 변환해준다.
	{
...
public:
	...
	void DrawRhombus64(LPDIRECTDRAWSURFACE lpDDS, int x, int y, COLORREF crColor);
